...
export default function YTMusicPlaylistPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const res = await fetch(`/api/yt-playlist/${id}?pageToken=${nextPageToken}`);
      if (res.ok) {
        const data = await res.json();
        ...
        <div className="max-w-6xl w-full mx-auto px-4 md:px-8 py-8">
          <div className="flex flex-col md:flex-row gap-8 items-center md:items-end mb-8">
            ...
            <div className="space-y-3 flex-1">
              ...
              <div className="h-4 bg-surface-container rounded w-48 animate-pulse" />
        ...
        <div className="flex flex-col md:flex-row gap-8 items-center md:items-end">
          ...
          <div className="flex flex-col gap-3 text-center md:text-left">
            ...
            <div className="flex items-center gap-2 justify-center md:justify-start text-[14px] leading-[20px] text-on-surface-variant">
              {playlist.author && (
                <>
                  <span className="font-semibold text-on-surface">{playlist.author}</span>
                  <span>•</span>
                </>
              )}
              <span>{playlist.trackCount || tracks.length} songs</span>
            </div>
